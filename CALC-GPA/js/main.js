...
function getRez() {
  ...
  for (let i = 1; i <= n; i++) {
    ...
    P_IN = +((val_vl1[count]).value).replace(',', '.') + 0.1016,
      P_OUT = +((val_vl2[count]).value).replace(',', '.') + 0.1016,
      T_IN = +((val_vl3[count]).value).replace(',', '.') + 273.15,
      T_OUT = +((val_vl4[count]).value).replace(',', '.') + 273.15;
